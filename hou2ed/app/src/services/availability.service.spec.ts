...
describe('batchUpdateAvailability', () => {
  test('should update multiple listings', async () => {
    ...
    const mockResults: AvailabilityResult[] = [
      { success: true, data: { listing_id: 'listing-1', beds_today: 5, beds_week: 0, waitlist_days: 0, last_confirmed: '2025-01-03T12:00:00Z' } },
      { success: true, data: { listing_id: 'listing-2', beds_today: 0, beds_week: 10, waitlist_days: 0, last_confirmed: '2025-01-03T12:00:00Z' } },
      { success: true, data: { listing_id: 'listing-3', beds_today: 3, beds_week: 5, waitlist_days: 7, last_confirmed: '2025-01-03T12:00:00Z' } },
    ];
  ...
  test('should handle partial failures', async () => {
    ...
    (supabase.rpc as jest.Mock)
      ...
      .mockResolvedValueOnce({ data: null, error: { message: 'Not found' } } as any);
...
describe('getListingsNeedingConfirmation', () => {
  test('should fetch listings needing confirmation', async () => {
    (supabase.auth.getUser as jest.Mock).mockResolvedValueOnce({
      data: { user: { id: 'user-123' } },
    } as any);

    const providerFromMock = {
      select: jest.fn().mockReturnThis(),
      eq: jest.fn().mockReturnThis(),
    ...
    const listingsFromMock = {
      ...
      lt: jest.fn().mockResolvedValueOnce({
        data: [
          { id: 'listing-1', name: 'Property A', last_confirmed: '2024-12-20T12:00:00Z' },
          { id: 'listing-2', name: 'Property B', last_confirmed: '2024-12-15T12:00:00Z' },
        ],
        error: null,
      }),
    };
    ...
    (supabase.from as jest.Mock)
      .mockReturnValueOnce(providerFromMock as any)
      .mockReturnValueOnce(listingsFromMock as any);
  ...
  test('should return empty array if not a provider', async () => {
    (supabase.auth.getUser as jest.Mock).mockResolvedValueOnce({
      data: { user: { id: 'user-123' } },
    } as any);
    ...
    (supabase.from as jest.Mock).mockReturnValueOnce(providerFromMock as any);
      ...
      fc.property(
        fc.array(
          fc.record({
            ...
            changes: fc.array(
              fc.record({
                field: fc.constantFrom('beds_today', 'beds_week', 'waitlist_days'),
                from: fc.nat({ max: 100 }),
                to: fc.nat({ max: 100 }),
              }),
              { maxLength: 3 }
        ...
        async (history) => {
          ...
          const expectedUpdates = history.filter(h => h.change_type === 'updated').length;
          const expectedConfirmations = history.filter(h => h.change_type === 'confirmed').length;
