...
export default function ListingCard({
  ...
  const getAvailabilityColor = () => {
    const bedsToday = listing.availability?.beds_today ?? 0;
    const waitlist = listing.availability?.waitlist ?? 0;
  ...
  const getAvailabilityText = () => {
    const bedsToday = listing.availability?.beds_today ?? 0;
    const bedsWeek = listing.availability?.beds_week ?? 0;
    const waitlist = listing.availability?.waitlist ?? 0;

    if (bedsToday > 0) {
      return `${bedsToday} beds available`;
    } else if (bedsWeek > 0) {
      return `${bedsWeek} beds this week`;
    } else if (waitlist > 0) {
    ...
    >
      ...
      <View style={styles.content}>
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            ...
            <Text style={styles.provider} numberOfLines={1}>
              {listing.provider?.full_name || 'Unknown Provider'}
            </Text>
          </View>
          {listing.verified && (
        ...
        <View style={styles.features}>
          {listing.eligibility?.family_status?.includes('families') && (
          ...
          {listing.accessibility?.mobility && listing.accessibility.mobility.length > 0 && (
            <View style={styles.featureChip}>
              <Text style={styles.featureText}>Accessible</Text>
            </View>
          )}
          {listing.rules?.pets?.toLowerCase().includes('allow') && (
            <View style={styles.featureChip}>
